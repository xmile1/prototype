...
// and avoid the problems of distributed computing.With this design, if a system needs to scale, 
// customers must buy bigger, more complex, and more expensive proprietary hardware with more processing power, 
// memory, and storage.Upgrades are also a challenge, as the organization must go through a lengthy acquisition process, 
// and then often take the system offline to actually make the change.This is all happening while the number of users continues to increase, 
// causing more and more strain and increased risk on the under - provisioned resources.




// NoSQL databases, on the other hand, usually support auto-sharding, meaning that they natively and automatically spread data across an 
// arbitrary number of servers, without requiring the application to even be aware of the composition of the server pool.
// Data and query load are automatically balanced across servers, and when a server goes down, it can be quickly and transparently replaced with 
...
// To put it simply, some operations will succeed while others fail. -- speed and scale over data integrity
